...
When building a Lavish review surface (and later the forked version), once the user has answered a question, do NOT show that question again on a subsequent update of the page.
Replace it with a compact "confirmed / answered" summary (or collapse it) so the surface only ever presents what still needs her input.

**Why:** She answered Q2 (parallel gaps), and a later page revision still rendered Q2 as an open question alongside the one remaining question — she found re-asking an already-settled question redundant and told me directly to remember it.
