...
description: "How the user wants lavish-axi review artifacts built — interactive radios, working submit, notifications"
metadata: 
  node_type: memory
  type: feedback
  scope: global
...
When building lavish-axi review surfaces for the user, default to these:

- **Interactive controls, not annotate-only.** Put real `<input type="radio">` / `<form>` choices in the artifact so she can pick options directly, rather than highlighting elements and annotating. (lavish "input" playbook.)
...
- **Notifications:** ping her when a response/needs-input is ready — see [[notify-on-response]].

**Why:** she'll reuse lavish and wants the basics (radios, working queue, notifications) to just work.
