...
description: Codex tier+effort are set separately and BOTH are per-machine; Sol defaults to `low` on your Mac, an unknown -p profile falls back SILENTLY instead of erroring, so read the run-log banner not the flag you passed
metadata:
  node_type: memory
  type: feedback
  scope: global
...
| --- | --- | --- |
| your Mac (CLI 0.146.0, verified 2026-08-17) | all three 5.6 tiers; **`gpt-5.6-sol` defaults to `low`** | `luna`(low) `terra`(high) `sol`(high) `solx`(xhigh) |
| a-collaborator's Windows (his ChatGPT account, CLI 0.147.0) | Sol 400s `not supported when using Codex with a ChatGPT account`; Terra is the ceiling | `luna` `terra` `terrax` `terramax` |
| your institution cluster (CLI 0.146.0) | base config is `gpt-5.6-sol`, and Sol runs fine | **none** — every `-p` falls back |
...
- Published API prices are **not what you pay** — Codex authenticates with a ChatGPT subscription, so usage draws on plan allowance. Use them as relative weights only. When the allowance is exhausted the CLI offers a paid upgrade: **never take it** — see [[no-extra-cash-without-permission]]. A limit reset can be a month out, so record what could not be reviewed instead of quietly shipping as if it had been.
