...
- Do NOT `rebase --abort` to tidy up: it discards every sync commit made since the rebase began, plus uncommitted work. Name the line first (`git branch <rescue> HEAD`) so nothing can be garbage-collected, and back up uncommitted files, then let the user decide between `--continue`, resetting `main` to the rescue ref, or something else.
