...
- `~/.claude` is the canonical config dir; `~/.claude1` (the active `CLAUDE_CONFIG_DIR`) mirrors it. CLAUDE.md, AGENTS.md, settings(.linux).json, and the tracked skills (email-drafter, codex-converge, codex-opinion, no-mistakes, scientific-figures) are all symlinks resolving into the repo. On Linux/WSL the repo lives at `~/claude-config/`, not `~/dotfiles/claude/`.
