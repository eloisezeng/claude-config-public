...
When the user has already approved the work ("go", "merge it", "do it"), a status report is a **checkpoint inside** the job, never the end of a turn. Ending a turn with "here is what I found, and here is what I will do next" leaves the session visibly idle while she waits — from outside, a plan and a stall are indistinguishable.
...
3. **Blocked on the user** — you name the single decision you need, and why no assumption is safe.
