...
This applies to YOURSELF, not only to delegates. A mutation-testing loop that rewrites a tracked file and restores it is the same contamination: in a repo whose code identity is a content hash over the tree, a concurrently-running suite sees `code_rev` change mid-run and the identity-dependent tests (checkpoint-resume, lineage gates) fail in a way that looks exactly like flakiness. Two such failures in your-research-project round 4 were traced to precisely this, and the clean re-run on a stable tree confirmed it. Serialize the mutation loop against the full suite, or give it its own checkout.
