...
- **The opposite reading is just as live, so "equivalent" is a CLAIM that needs the same construction: before calling a survivor equivalent, build the state the mutated code exists for.** Measured 2026-09-17 on a memory-store refresher: two mutants survived, one was a genuinely equivalent fast path, and chasing the other — an explicit fetch refspec that "looked decorative" — found a store that would have stopped following its branch for ever while advising a repair that writes config already present (`[[a-branchs-upstream-resolves-through-the-fetch-refspec]]`). The two readings are not symmetric in cost: an unobservable property costs a test you did not need, and a wrongly dismissed survivor costs the defect it was the only witness to. Spend the reachability argument on the dismissal, not on the keep.
