...
- `claude` (default) → **your-company** account (you@example.com), auth via macOS Keychain entry `Claude Code-credentials` / acct `you`. Untouched by the setup.
...
**Why this approach:** macOS keeps ONE shared Keychain login across all `CLAUDE_CONFIG_DIR` values, so two OAuth subscription logins can't coexist there — the env-var token bypasses the Keychain. Verified empirically on this machine that running with `CLAUDE_CODE_OAUTH_TOKEN` set does NOT delete or overwrite the shared `Claude Code-credentials` entry (the reported claude-code#37512 bug does NOT reproduce here), so `claude1` is safe for your-company login.
