...
When you critique a visual mockup (e.g. lavish-axi review surface) during brainstorming, every concrete change you ask for must be carried into the written spec/design doc — not left only in the mockup or the chat.

**Why:** In the TO-DO-13 session, two mockup-approved details — the rename pencil mirrored via `transform: scaleX(-1)`, and CSS-drawing the take-× delete badge as two crossing bars for true centering — were discussed and approved in lavish mockup but never made it into the spec → plan → implementation, so they silently didn't ship. The Decision log captured the big conceptual choices but not the fine visual critiques from the mockup-refinement rounds.
