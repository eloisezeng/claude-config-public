---
name: mermaid-id-collision-lavish
description: Mermaid v11 startOnLoad breaks multi-diagram lavish artifacts (same-ms SVG id collision); render manually with unique ids
metadata: 
  node_type: memory
  type: reference
  scope: global
...
**How to apply:** in lavish/HTML artifacts with multiple `<pre class="mermaid">` blocks, set `startOnLoad: false` and render each block explicitly with a unique id:
