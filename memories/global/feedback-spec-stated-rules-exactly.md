...
description: When the user states a precise display/counting/layout/timing rule, implement it EXACTLY and pin it with a test — approximate implementations get re-filed verbatim
metadata:
  node_type: memory
  type: feedback
  scope: global
...
2. Before claiming done, **reproduce the exact example she cited** end-to-end (the specific video, the specific segment size) and confirm the rule holds *there*, not just in general. Ties into [[execution-verification-prefs]].
