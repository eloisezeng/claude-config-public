...
When the user reviews via `lavish-axi` and sends annotations/questions, she wants each answer **model-routed by complexity** rather than all on one model:
...
**How to apply:** prefix every Lavish reply with the model used, e.g. `[ model: Sonnet ]`, and tell her she can reply `haiku` / `sonnet` / `opus` on that thread to re-answer with a different model; honor such overrides. Lavish itself has no model logic — routing is a Claude Code decision (the model is the session's, not Lavish's). Consume annotations with PR #111's `lavish-axi stream <file> --once` (one message per annotation) and thread replies with `--reply-to <id>`.
