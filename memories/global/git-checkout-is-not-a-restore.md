...
In an **auto-committing** repo the failure inverts and gets worse: `~/dotfiles/claude` commits and pushes on a ~1-minute launchd timer, so a bad working-tree experiment is often already the index AND `HEAD` by the time you undo it. `git checkout --` then restores *the experiment*, reports nothing, and the tree looks repaired. Measured 2026-09-03: a `json.dumps(indent=1)` rewrite of an `indent=2` manifest was committed by the daemon mid-flight; the restore returned the reformatted file, and a second append then produced duplicate entries. Recover by reading the good bytes out of an OLDER object (`git show HEAD~2:<path>`), proving the formatter round-trips byte-identically against it, and re-deriving the file — never by `checkout`. Count the entries before and after every manifest edit; a diff line-count is the wrong instrument when a formatter has touched every line.
