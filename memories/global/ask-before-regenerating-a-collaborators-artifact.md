...
**Measured 2026-08-16 (your-research-project).** A pilot retrained the `your_transfer_baseline` baseline at 2500 epochs (~5.5 of 15.3 GPU-h). your mentor had already trained it at that budget off-repo; the evidence was sitting in `final_error_matrix.csv`, whose film row matched the retrain to ~2.5% median. When challenged, the defence was a whole-filesystem checkpoint census (278 e200, 267 e2, zero e2500) — a probe that **cannot see a collaborator's off-repo run at all**, so it established nothing about whether the training had happened, only that its weights were not on this box. your instruction: ask you whether training is necessary before doing it.
